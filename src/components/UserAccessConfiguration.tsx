...
export type UserAccessConfig = {
  ...
  remote: boolean
  /**
   * Access PIN as a string of digits, or `null` when no PIN is set. The
...
type Props = {
  ...
  onBack: () => void
  onClose: () => void
  onSave: (cfg: UserAccessConfig) => void
  /**
   * Open the Edit Access Code sub-screen. The current local draft is handed
   * back to the parent so the rest of the form's unsaved edits survive the
   * round-trip through the PIN editor.
   */
  onEditPin: (currentDraft: UserAccessConfig) => void
}

/**
 * User Access Configuration — structurally identical to Bypass Configuration
 * (same layout, same controls, same conditional disable rules), just with
 * the copy and PIN edit flow scoped to "access" instead of "bypass".
 *
 * The two screens are kept as parallel files rather than a shared component
 * so each can evolve independently — they share enough today, but the
 * user-access flow is likely to grow per-role options that bypass won't.
 */
export function UserAccessConfiguration({
  value,
  ...
  const t = useT()
  const [draft, setDraft] = useState<UserAccessConfig>(value)
  const dirty = JSON.stringify(draft) !== JSON.stringify(value)

  const set = <K extends keyof UserAccessConfig>(
    key: K,
    v: UserAccessConfig[K],
  ) => setDraft((d) => ({ ...d, [key]: v }))
  ...
  return (
    ...
    >
      ...
      {/* Set Access PIN Code row. Disabled when PIN Code authentication
          itself is unchecked — there's no point editing a PIN that won't
          be used. */}
      <button
        type="button"
